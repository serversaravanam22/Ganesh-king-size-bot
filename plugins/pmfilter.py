    ...
    async def give_filter(client, message):
        if message.chat.id != SUPPORT_CHAT_ID:
            ...
            if total_results == 0:
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔍 ᴊᴏɪɴ ᴀɴᴅ ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url=f"https://telegram.me/Tamil_Movies_Request_Group_AK")]]))
else:   
...
if PM_SEARCH == True:
    ...
    async def pm_text(bot, message):
        ...
        await message.reply_text(
             ...
             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://telegram.me/Tamil_Movies_Request_Group_AK")]])
        )
    ...
    async def pm_text(bot, message):
        ...
        await message.reply_text(
             text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ ʙʏ ᴄʟɪᴄᴋɪɴɢ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ 👇</b>",   
             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝HD & OTT Prints Only ", url=f"https://telegram.me/Tamil_Movies_Request_Group_AK")],
                                                [InlineKeyboardButton("📝Theatre & HD & OTT Prints ", url=f"https://telegram.me/+g8WRwjUrCHE4OTA1")]])
        )
        await bot.send_message(
            chat_id=LOG_CHANNEL,
            text=f"<b>#𝐏𝐌_𝐌𝐒𝐆\n\nNᴀᴍᴇ : {user}\n\nID : {user_id}\n\nMᴇssᴀɢᴇ : {content}</b>"
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("🔍 ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url="https://telegram.me/Tamil_Movies_Request_Group_AK")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # Thanks to @I_Am_MrAK for fixing this 
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔍 𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦 𝐆𝐫𝐨𝐮𝐩', url='https://telegram.me/Tamil_Movies_Request_Group_AK'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'Seaching for {search} 🔎', url=f"https://telegram.me/Tamil_Movies_Request_Group_AK")]]) 
            )
            search = search.lower()
            find = search.split(" ")
            search = ""
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if not g_s:
            ...
            button = [[
                       InlineKeyboardButton("📝 ʀᴇǫᴜᴇꜱᴛ ʜᴇʀᴇ", url=f"https://telegram.me/Tamil_Movies_Request_Group_AK")
            ]]
            if NO_RESULTS_MSG:
